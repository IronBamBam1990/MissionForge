...
def generate_init_server(config: MissionConfig, enemy_faction_data=None) -> str:
    ...
    # ── 2. TERRAIN SNAP + WATER SAFETY (all units + vehicles from SQM) ──
    L.append("// Terrain snap all SQM units + move out of water")
    L.append("private _snappedCount = 0; private _waterCount = 0;")
    L.append("{")
    L.append("\tprivate _origPos = getPosATL _x;")
    L.append("\tif ([_origPos] call IBC_fnc_isWater) then {")
    L.append("\t\tprivate _safePos = [_origPos] call IBC_fnc_findSafePos;")
    L.append("\t\t_x setPosATL [_safePos select 0, _safePos select 1, 0];")
    L.append("\t\t_waterCount = _waterCount + 1;")
    L.append("\t} else {")
    L.append("\t\t_x setPosATL [_origPos select 0, _origPos select 1, 0];")
    L.append("\t};")
    L.append("\t_snappedCount = _snappedCount + 1;")
    L.append("} forEach allUnits;")
    L.append('diag_log format ["IBC: %1 units snapped, %2 moved from water", _snappedCount, _waterCount];')
    L.append("")
    L.append("// Snap vehicles out of water too")
    L.append("{ if ([getPosATL _x] call IBC_fnc_isWater) then {")
    L.append("\tprivate _vSafe = [getPosATL _x] call IBC_fnc_findSafePos;")
    L.append("\t_x setPosATL [_vSafe select 0, _vSafe select 1, 0.3];")
    L.append("\tdiag_log format [\"IBC: Vehicle %1 moved from water to %2\", typeOf _x, _vSafe];")
    L.append("} } forEach vehicles;")
    L.append("")
    L.append("// Delayed re-check after 10s (catches late-spawning units)")
    L.append("[] spawn {")
    L.append("\tsleep 10;")
    L.append("\t{")
    L.append("\t\tif (alive _x && {[getPosATL _x] call IBC_fnc_isWater}) then {")
    L.append("\t\t\tprivate _safePos = [getPosATL _x] call IBC_fnc_findSafePos;")
    L.append("\t\t\t_x setPosATL [_safePos select 0, _safePos select 1, 0];")
    L.append('\t\t\tdiag_log format ["IBC: Delayed fix - unit %1 moved from water", typeOf _x];')
    L.append("\t\t};")
    L.append("\t} forEach allUnits;")
    L.append("\t{")
    L.append("\t\tif ([getPosATL _x] call IBC_fnc_isWater) then {")
    L.append("\t\t\tprivate _vSafe = [getPosATL _x] call IBC_fnc_findSafePos;")
    L.append("\t\t\t_x setPosATL [_vSafe select 0, _vSafe select 1, 0.3];")
    L.append('\t\t\tdiag_log format ["IBC: Delayed fix - vehicle %1 moved from water", typeOf _x];')
    L.append("\t\t};")
    L.append("\t} forEach vehicles;")
    L.append("};")
    L.append("")
    ...
    # ── 11b. BLUFOR AI BEHAVIOR (bots replacing absent players) ──
    # AI bots board available vehicles, then move to objectives.
    # Groups near vehicles mount up first, then get MOVE waypoints.
    obj_positions = [_sqf(obj.position) for obj in config.markers.objectives]
    if obj_positions:
        obj_arr = "[" + ",".join(obj_positions) + "]"
        L.append("// === BLUFOR AI — Mount Vehicles & Execute Mission ===")
        L.append("[] spawn {")
        L.append("\tsleep 20;  // wait for players to connect")
        L.append(f"\tprivate _objectives = {obj_arr};")
        L.append("")
        L.append("\t// Collect empty WEST vehicles (cars, APCs, trucks — not static weapons)")
        L.append('\tprivate _availableVehicles = vehicles select {')
        L.append('\t\talive _x')
        L.append('\t\t&& {count (crew _x) == 0}')
        L.append('\t\t&& {side _x == west || {side _x == civilian}}')
        L.append('\t\t&& {_x isKindOf "Car" || _x isKindOf "Tank" || _x isKindOf "Wheeled_APC_F" || _x isKindOf "Truck_F"}')
        L.append("\t};")
        L.append('\tdiag_log format ["IBC: Found %1 empty vehicles for AI teams", count _availableVehicles];')
        L.append("")
        L.append("\t// Process each AI-only BLUFOR group")
        L.append("\t{")
        L.append("\t\tprivate _grp = _x;")
        L.append("\t\tprivate _hasPlayer = {isPlayer _x} count (units _grp) > 0;")
        L.append("\t\tif (!_hasPlayer && side _grp == west && count (waypoints _grp) <= 1) then {")
        L.append("")
        L.append("\t\t\t_grp setBehaviour \"AWARE\";")
        L.append("\t\t\t_grp setCombatMode \"YELLOW\";")
        L.append("\t\t\t_grp setSpeedMode \"NORMAL\";")
        L.append("")
        L.append("\t\t\t// Try to board nearest available vehicle")
        L.append("\t\t\tprivate _leader = leader _grp;")
        L.append("\t\t\tprivate _mounted = false;")
        L.append("\t\t\tif (count _availableVehicles > 0) then {")
        L.append("\t\t\t\t// Find closest vehicle to group leader")
        L.append("\t\t\t\tprivate _bestVeh = objNull;")
        L.append("\t\t\t\tprivate _bestDist = 99999;")
        L.append("\t\t\t\t{")
        L.append("\t\t\t\t\tprivate _d = _leader distance _x;")
        L.append("\t\t\t\t\tif (_d < _bestDist) then { _bestDist = _d; _bestVeh = _x; };")
        L.append("\t\t\t\t} forEach _availableVehicles;")
        L.append("")
        L.append("\t\t\t\tif (!isNull _bestVeh && {_bestDist < 500}) then {")
        L.append("\t\t\t\t\t// Remove from available pool")
        L.append("\t\t\t\t\t_availableVehicles = _availableVehicles - [_bestVeh];")
        L.append("")
        L.append("\t\t\t\t\t// Board: leader drives, rest as cargo")
        L.append("\t\t\t\t\t_leader assignAsDriver _bestVeh;")
        L.append("\t\t\t\t\t[_leader] orderGetIn true;")
        L.append("\t\t\t\t\tprivate _seats = _bestVeh emptyPositions \"cargo\";")
        L.append("\t\t\t\t\tprivate _idx = 0;")
        L.append("\t\t\t\t\t{")
        L.append("\t\t\t\t\t\tif (_x != _leader && _idx < _seats) then {")
        L.append("\t\t\t\t\t\t\t_x assignAsCargo _bestVeh;")
        L.append("\t\t\t\t\t\t\t[_x] orderGetIn true;")
        L.append("\t\t\t\t\t\t\t_idx = _idx + 1;")
        L.append("\t\t\t\t\t\t};")
        L.append("\t\t\t\t\t} forEach (units _grp);")
        L.append("")
        L.append("\t\t\t\t\t// First waypoint: GET IN vehicle")
        L.append("\t\t\t\t\tprivate _wpGetIn = _grp addWaypoint [getPos _bestVeh, 10];")
        L.append("\t\t\t\t\t_wpGetIn setWaypointType \"GETIN\";")
        L.append("\t\t\t\t\t_wpGetIn setWaypointCompletionRadius 20;")
        L.append("")
        L.append("\t\t\t\t\t_mounted = true;")
        L.append('\t\t\t\t\tdiag_log format ["IBC: AI group %1 boarding %2 (%3m away)", _grp, typeOf _bestVeh, round _bestDist];')
        L.append("\t\t\t\t};")
        L.append("\t\t\t};")
        L.append("")
        L.append("\t\t\t// Add MOVE waypoints to each objective")
        L.append("\t\t\t{")
        L.append("\t\t\t\tprivate _wp = _grp addWaypoint [_x, 30];")
        L.append("\t\t\t\t_wp setWaypointType \"MOVE\";")
        L.append("\t\t\t\t_wp setWaypointSpeed \"NORMAL\";")
        L.append("\t\t\t\t_wp setWaypointFormation \"WEDGE\";")
        L.append("\t\t\t\t_wp setWaypointCompletionRadius 50;")
        L.append("\t\t\t\tif (_mounted) then {")
        L.append("\t\t\t\t\t// Dismount before last objective (fight on foot)")
        L.append("\t\t\t\t\tif (_forEachIndex == count _objectives - 1) then {")
        L.append("\t\t\t\t\t\tprivate _wpDismount = _grp addWaypoint [_x, 80];")
        L.append("\t\t\t\t\t\t_wpDismount setWaypointType \"GETOUT\";")
        L.append("\t\t\t\t\t\t_wpDismount setWaypointCompletionRadius 80;")
        L.append("\t\t\t\t\t};")
        L.append("\t\t\t\t};")
        L.append("\t\t\t} forEach _objectives;")
        L.append("")
        L.append("\t\t\t// Final waypoint: search and destroy at last objective")
        L.append("\t\t\tprivate _sadWp = _grp addWaypoint [_objectives select (count _objectives - 1), 50];")
        L.append("\t\t\t_sadWp setWaypointType \"SAD\";")
        L.append('\t\t\tdiag_log format ["IBC: AI group %1 — %2 waypoints, mounted=%3", _grp, count waypoints _grp, _mounted];')
        L.append("\t\t};")
        L.append("\t} forEach allGroups;")
        L.append("};")
        L.append("")
